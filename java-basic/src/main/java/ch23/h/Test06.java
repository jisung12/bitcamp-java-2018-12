...
public class Test06 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // HttpGet 클래스
    // => www.apache.org 사이트에서 제공하는 HttpComponent 라이브러리에 들어있는 클래스이다.
    // => 사용하려면 먼저 외부 라이브러리를 프로젝트로 가져와야 한다.
    //    1) mvnrepository.com에서 apache httpClient 키워드로 검색한다.
    //      2) 최신 라이브러리 정보를 build.gradle의 dependencise {} 블록에 추가.
    //      3) 콘솔에서 "gradle eclipse"를 실행하여 .classpath 파일을 갱신한다.
    //        이때 외부 라이브러리를 자동으로 다운받을 것이다.
    //      4) 이클립스 에디터에서 해당 프로젝트를 refresh한다.
    ...
    // InputStream을 가지고 데이터를 읽으려면 다시 입출력 코딩을 작성해야 한다.
