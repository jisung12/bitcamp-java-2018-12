...
public class Test06 {
  public static void main(String[] args) {
    ...
    // 2) InputStream은 바이트 단위로 읽는 기부ㄴ이 있다.
    //     바이트 단위로 읽어서 int나 문자열로 바꾸려면 또 코딩해야하는 불편함이 있다.
    //     이런 불편함을 줄이기 위해 자바에서는 바이트를 개발자가 원하는 값으로 
    //     바꿔주는 기능을 제공한다. 그런 기능이 들어 있는 도구가 java.util.Scanner 이다,
    java.util.Scanner keyboard = new java.util.Scanner(in);
    
    // 사용자에게 입력하라고 간단히 출력.
    System.out.println("이름을 입력하세요 : ");
    
    // 3)Scanner에 들어있는 nextLine()은 
