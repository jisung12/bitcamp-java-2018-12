...
public class Test02 {
  ...
  // => 직접적으로 스레드가 아니기 때문에 실행할 때는 Thread의 도움을 받아야한다.
  ...
  public static void main(String[] args) {
    ...
    // OS의 CPU 스케줄링 정책에 따라 스데르다 실행된다.
    // 즉 JVM에서 스레드를 실행하는 것이 아니라 OS가 실행한다.
    // 결론!
    // => 똑같은 자바의 스레드 코드가 OS에 따라 실행 순서가 달라질 수 있다.
    
    // 우선 순위로 조정하면 되지 않나?
    // => Windows OS의 경우 우선순위(priority) 값이 실행 순서나 실행 회수에 큰 영향을 끼치지 않는다.
