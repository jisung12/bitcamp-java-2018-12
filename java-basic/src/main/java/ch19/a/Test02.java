...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // 예1 :
    // => Car 클래스를 상속받아 익명 클래스를 정의한다.
    // => 인스턴스를 생성할때 수퍼 클래스인 Car의 기본 생성자를 호출한다.
    Car obj = new Car() {
      @Override
      public void run() {
        System.out.println(this.model + ": 달려라 달려라");
      }
    }; // new 명령은 인스턴스를 생성하는 명령이기 때문에 문장 끝에 세미콜론(;)을 붙여야한다.
    obj.run();
  ...
  public static void m1() {
    // 스태틱 맴버는 오직 같은 스태틱 맴버만 사용할 수 있다.
    A obj = new A();
    
    // 스태틱 멤버는 인스턴스 주소를 담는 this라는 built-in 변수가 없기 때문에
    // 인스턴스 맴버(필드, 메서드, inner 클래스)를 사용할 수 없다.
