// static nested class의 문법 응용
package ch19.d;

// 상수를 관리하는 용도로만 사용될 클래스라면 서브 클래스를 만들지 못하도록 막아야한다.
// => final 제한자를 사용하라!
public final class Category {

  // static nested class 문법이 필요한 이유?
  // 작은 클래스를 패키지 맴버 클래스로 만들면 소스 파일이 너무 많이 생기기 때문에 관리하기가 번거롭다.
  // => 예 :
  //   Computer.java, Mouse.java, Appliance.java, Book.java
  
  // =>이렇게 작은 크기의 클래스가 많은 경우 
  // 어느 한 클래스 안에 static 중첩 클래스로 작성하는 것이
  // 소스 파일을 관리하기 쉽게 도와준다.
  
  // 분류의 각 계층을 static 중첩 클래스로 선언하여 관리한다.
  // => 보통 클래스 이름은 대문자로 시작해야한다.
  // => 그런데 OGNL 표기법에서 하위프로퍼티(필드)를 지정하는 것처럼 보이게 하려고
  //     일부러 소문자로 이름을 지었다
