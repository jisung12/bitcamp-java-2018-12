...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // 반드시 예외처리를 해야한다.
    // try ~ catch로 처리하든 throws 절을 메서드 선언부에 붙이든 둘 중 하나는 해야한다.

    for(int i = 0; i < list.size(); i++) {
      String name = getName(i);
      // 예외 상황을 검사하기 위해 getName()으로 리턴 값을 확인한다.
      // null이면 예외가 발생했다는 것이다.
      // => 문제는 예외 상황일 때 메서드가 리턴하는 값이 정상 값일 수 있다는 것이다.
      if (name == null) {
   ...
   // int result = divide(1000, 0); //OK! 예외임을 알 수 있다.
    int result = divide(191919, -1);// 정상임에도 불구하고 리턴값이 -191919라서 오류로 판단.
    if (result == -191919) { // 이것이 리턴값으로 오류를 전달할 때의 문제이다.
  ...
  public static int divide(int a, int b) {
    if (b == 0) {
      // 호출자에게 어떤 값으로 알려줘라 오류를 전달할 수 있을까?
