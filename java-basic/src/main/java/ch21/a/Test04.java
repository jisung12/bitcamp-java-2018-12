...
public class Test04 {
  ...
  public static void main(String[] args) {
    ...
    // => 문법
    //    try { 예외 발생할 수 있는 코드 } catch (Exception e) { 예외 처리 코드 }
    try {
    System.out.println(list.get(3)); 
    // 예외가 발생할 수 있는 코드, 예외 발생시 예외 정보가 들어있는 객체를 catch 블록으로
    
    } catch (Exception e) {
      // try에서 던진 예외 객체를 파라미터로 받는다.
    ...
    // "예외처리 문법의 존재 이유"
    // => 예외가 발생하더라도 시스템을 멈추지 않고 계속 실행되게 하는 기술이다.
