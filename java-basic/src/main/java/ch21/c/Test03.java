...
public class Test03 {
  public static void main(String[] args) 
      ...
      SecurityException,     // 즉시 이 메서드의 실행을 멈추고 
      InstantiationException, // 이 메서드를 호출한 쪽에 예외 정보를 던진다
      IllegalAccessException, // 이메서드를 호출한 쪽에 예외 정보를 보고한다.
      InvocationTargetException{ // main()의 호출자? JVM이다.

    // Exception 계열의 예외 처리 방법 
    // 1) try ~ catch로 예외 받기
    //    try {예외 발생 코드} catch (예외 파라미터) {예외처리 코드}
    // 2) 호출자에게 예외 처리를 떠넘기기
