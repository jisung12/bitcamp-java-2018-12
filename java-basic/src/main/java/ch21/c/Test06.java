...
public class Test06 {
  public static void main(String[] args) {
    ...
    System.out.println(list.get(3));// 예외가 발생하면 즉시 실행을 중지하고 호출자에게 보고한다.다
    // main() 메서드의 호출자는 JVM이기다. JVM은 예외를 보고 받으면 즉시 실행을 종료한다.
    // 따라서 다음 코드는 실행되지 않는다.
    } catch (RuntimeException e) {
      // 여기서 예외를 처리했기 때문에 main() 호출자에게 다로 보고하지 않는다.
