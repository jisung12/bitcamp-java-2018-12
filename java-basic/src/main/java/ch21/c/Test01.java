...
public class Test01 {
  public static void main(String[] args) {
    ...
    // => 리턴 값은 로딩한 클래스의 정보이다.
    // => 이 메서드는 파라미터에서 지정한 클래스를 찾지못했을 때 
    //    ClassNotFoundException 예외를 발생시킨다.
    // => 이 예외는 Exception의 자식클래스이다.
    //    Exception의 자식클래스이고 RuntimeException 계열이 아닐 경우
    //    try ~ catch 블록으로 반드시 예외르 받도록 처리해야 한다.
    // getConstructor() 메서드
    // => 생성자를 찾아 리턴한다
   // => 만약 지정된 생성자를 찾지 못하면 NoSuchMethodException 예외가 발생한다
    ...
    // newInstance() 메서드
    // => 생성자를 이용하여 인스턴스를 만든다.
    // => 만약 인스턴스를 만들지 못한다면 InstantiationException 등의 예외가 발생.
    //    이 예외도 Exception 계열의 메서드이다.
    //    즉 반드시 try ~catch 를 사용하여 예외를 처리해야 한다.
    ...
    //    JVM은 프로그램을 멈추기 때문에 애플리케이션을 개발할때 이런 예외도 처리하도록 하라.
    
    // execute() 메서드
    // => PlusCommand나 DivideCommand의 execute() 메서드는 
    //     내부적으로 NumberFormatException, ArithmaticException이 발생한다.
    // => 그런데 이들 예외는 RuntimeException 계열이라서
    //   catch 블록에서 예외를 반드시 받아야 하는 것은아니다.
    // => 그래서 아래의 try ~ catch 블록에 이들 예외를 처리하는 catch 블록이 없는 것이다.
    try {
      // 사용자로부터 로딩할 클래스 이름을 입력 받는다.
      System.out.println("실행할 Command 클래스 이름을 입력하세요>");
     ...
     command.execute();
      ...
      // 예외를 발생시키는 방법 : 클래스 이름을 입력할 때> ch21.c.ok
      System.out.println("해당 클래스를 찾지 못했습니다.");
      
    } catch (NoSuchMethodException e) {
   // 예외를 발생시키는 방법 : 클래스 이름을 입력할 때> java.lang.String
