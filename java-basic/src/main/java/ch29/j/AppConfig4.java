...
public class AppConfig4 {
  ...
  // 이때 @Bean 애노테이션을 붙여 이메서드를 호출 하라고 스프링 IoC 컨테이너에게 알려줘야 한다.
  // 스프링 IoC 컨테이너는 이 메서드를 호출한 후 그 리턴 값을 메서드 이름으로 저장한다.
  // 만약 @Bean 애노테이션이 따로 이름을 지정한다면 그 이름으로 리턴 값을 저장한다.
  //
  @Bean
  public BlackBox blackBox() {
    // 이 메서드의 이름이 리턴 값을 저자아는 이름으로 사용되기 때문에
    // 메서드의 이름을 명사 형태로 짓는다.
...
//스프링 IoC 컨테이너로부터 파라미터로 주입받을 수 있다.
  @Bean
