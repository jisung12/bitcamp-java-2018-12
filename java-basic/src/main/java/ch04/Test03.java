...
public static void main(String[] args) {
 ...
 //     그래서 형변환(type conversion)을 '타입 캐스팅'(type castion)이라고도 부른다.
 // ==> 형변환 문법 = (새로만들 메모리의 타입) 값
 // ==> 다음과 같이 개발자가 형변환을 명시하는 경우, 
 //     '명시적 형변환'이라고 한다 (타입이 변하지 않음)
 ...
 // 다르다면 내부적으로 두 피연산자의 값을 같은 타입으로 만든 후에 계산해야한다.
 ...
 //byte b3 = b1 + b2; // 암시적 형변환에 의해 b1, b2의 값은 int 타입의 임시 메모리에 저장.
                          // 그런 후 계산된다. int와 int의 결과는 당연 int이다.
                          // 따라서 컴파일 오류!
 ...
 // char c3 = c1 +c2; //컴파일 오류!
  ...
  double d1 = f3 + f4; // float과 float의 계산 결과는 float이다.
                           // d1에 저장되기 전에 자릿수(7자리)를 초과한 값은 오차값으로 저장.
  //그래서 d1을 출력하면 오차가 있는 값이 출력되는 것이다.
  System.out.println(d1);
  
  // 해결책!
  // => 계산하기 전에 두 큰 팀으로 형볗놘하라.
  double d2 = (double)f3 + (double)f4;
  // 그런데 출력 결과를 보면 예상 결과와 다르게 나온다.
  // 이유? float을 double로 형변환할때 오차가 이미 생긴다.
