...
public class Test06 {
  ...
  public static void main(String[] args) {
    ...
    //    - 데이터가 같은지 비교할 때 사용한다.
    //    - 즉 모든 데이터를 일일히 비교하는 대신에 미리 생성된 정수 값을 비교함으로써
    //      빠르게 두 값이 같은지 알아낼 수 있다.
    //    - 매우 낮은 확률이지만 데이터가 다르더라도 같은 정수 값이 나올 수 있다.
    //      큰 데이터를 특별한 계산 공식을 통해 4바이트 정수 값으로 표현한다는 것은 
    //      언제든 다른 데이터에 대해 같은 값이 나올 가능성을 내포하는 것이다.
    //      다만 확률이 얼마나 낮은가의 문제이다.
    //      다른 데이터에 대해 같은 정수 값이 나올 확률이 낮을 수록 안심하고 사용할 수 있다.
    //    - 현실과 비교하면 주민번호와 같다.
    //      본인 여부를 확일할 때 주민번호로 확인하면 매우 빠르게 결과를 알 수 있다.
    //      원래는 본인인지 알아내려면 DNA 검사를 해야한다. (오래걸린다) 그러나 지문, 주민번호를 사용하면 빠르게 알 수 있다.
    //    - 그래서 hash 값을 "디지털 지문"이라 부른다.
    //    - 목적 :
    //        => 데이터가 같은지 빠르게 비교하기 위함.
    //    - 응용 :
    //        => 본인 여부 확인 인증서 및 파일의 위변조를 검사하는 용도. 
    //                               예 1)  => git에서 커밋할때 고유번호를 붙이는게 해시 값이다.
    //      예 2) 파일 다운로드 사이트에서 제공하는 해시 값
    //      예 3) 파일 공유사이트에서 파일을 구분할 때 해시 값 사용.
