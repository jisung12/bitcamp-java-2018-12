...
public class ApplicationContext {
  ...
  public ApplicationContext(Class<?> configClass) throws Exception {
    ...
    Constructor<?> c = configClass.getConstructor(); // 클래스의 생성자를 담는다
    Object config = c.newInstance(); // 담은 생성자의 새 인스턴스를 Object에 담는다 
    
    // 2) Bean 애노테이션이 붙은 메서드를 모두 찾는다.
    ArrayList<Method> factoryMethods = new ArrayList<>(); // 메서드를 담을 ArrayList 준비
    
    Method[] methods = configClass.getMethods(); // 클래스의 메서드를 얻어 Method 배열에 담는다
    for (Method m : methods) { // 메서드를 반복문을 이용하여 얻는다
      if (m.getAnnotation(Bean.class) != null) // 만약 메서드의 애노테이션 Bean이 null이 아니면 (애노테이션이 Bean이면)
        factoryMethods.add(m); // 위에서 준비한 factoryMethods 배열에 메서드를 추가한다.
    }
    
    // 3) 팩토리 메서드를 호출하여 그 리턴 값을 빈 컨테이너에 보관한다.
    while (factoryMethods.size() > 0) { // 메서드들을 담은 factoryMethods 배열의 size가 0 이상이면 (즉, 있으면)
    ...
    ComponentScan componentScan = configClass.getAnnotation(ComponentScan.class);  // 클래스에서 ComponentScan 애노테이션인 클래스를 담는다.
    if (componentScan != null) { // 위에서 담은 ComponentScan이 null이 아니면(있으면)
      String[] packages = componentScan.basePackages(); //String 배열의 packages에  componentScan의 basePackages를 담는다.
      for (String packageName : packages) { // 위 String배열의 package를 반복문 돌린다.
        prepareComponent(packageName); //prepareComponent 메서드에 넣는다.
      }
    }
  }
  ...
  public void prepareComponent(String packageName) throws Exception {
    ...
    // => 또한 중첩 클래스도 제외한다.
    findClasses(packageDir, packageName); // findClasses 메서드에 packageDir과 packageName을 넣는다
  ...
  private void createComponent() throws Exception { 
    for (Class<?> clazz : classes) { // 반복문으로 클래스를 추출 
      // 클래스에서 Component 애노테이션 정보를 추출한다.
      Component compAnno = clazz.getAnnotation(Component.class); // 반복문 동안 Component 애노테이션의 클래스만 Compnent에 담는다.
      
      if (compAnno == null) //component 애노테이션이 null이면 
        continue; // 계속한다
      
      // Component 애노테이션이 붙은 클래스에 대해 인스턴스를 생성한다.
      Object obj = createInstance(clazz); //createInstance 메서드에 클래스를 담아 Object에 저장한다.
  ...
  private Object createInstance(Class<?> clazz) throws Exception {
    ...
    // => 기본 생성자를 알아낸다.
    try { 
      Constructor<?> defaultConstructor = clazz.getConstructor(); // 클래스의 생성자를 담는다
      return defaultConstructor.newInstance(); // 담은 생성자의 새 인스턴스를 return한다.
    } catch (Exception e) {
    ...
    Constructor<?>[] constructors = clazz.getConstructors(); // 클래스의 생성자를 담는다.
    for (Constructor<?> c : constructors) { // 반복문으로 생성자를 꺼낸다.
      // => 생성자를 호출하려면 먼저 어떤 타입의 파라미터가 필요한지 알아야 한다.
      Class<?>[] paramTypes = c.getParameterTypes(); // 생성자의 파라미터 타입을 얻어 담는다.
      
      // => 생성자가 요구하는 타입의 파라미터 값이 저장소에 있는지 찾아 본다.
      Object[] paramValues = getParameterValues(paramTypes);  // 담은 파라미터 타입을 getParameterValues 메서드를 통해 
  ...
  private Object[] getParameterValues(Class<?>[] paramTypes) {
    ...
    if (values.size() == paramTypes.length) // values의 size가 파라미터 타입의 length와 같다면
